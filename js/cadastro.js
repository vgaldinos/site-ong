function renderCadastro() {
    return `
		...
		<!-- Checkbox Oculto do Modal Nativo -->
		<input type="checkbox" id="modal-confirmacao" class="modal-checkbox">
		
		<div class="modal-overlay">
			<div class="modal-feedback-sucesso">
				<label for="modal-confirmacao" class="btn-fechar-modal">&times;</label>
				...
				<a href="#inicio" class="btn-voltar-inicio" onclick="document.getElementById('modal-confirmacao').checked = false">Voltar para o Início</a>
			</div>
		</div>
    `;
}
...
window.inicializarCadastro = function() {
    ...
    formulario.addEventListener('submit', function(evento) {
        ...
        if (formularioValido) {
            ...
            // ==========================================================
            // INTEGRAÇÃO COM A BIBLIOTECA EXTERNA SWEETALERT2
            // ==========================================================
            if (typeof Swal === 'function') {
                Swal.fire({
                    title: 'Excelente!',
                    text: 'Seu cadastro de apoiador foi processado e salvo localmente!!',
                    icon: 'success',
                    confirmButtonColor: '#1a4473',
                    confirmButtonText: 'Visualizar Comprovante'
                }).then((result) => {
                    if (result.isConfirmed) {
                        const modalCheckbox = document.getElementById('modal-confirmacao');
